...
export default function BloqueiosPage() {
  ...
  function renderBlock(block: BlockedSlot, index: number, past = false) {
    const prof = professionals.find((p) => p.id === block.professional_id);
    const dayOff = isDayOff(block);

    return (
      <BlockCard key={block.id} $index={index} $past={past}>
        <BlockIconWrap $past={past}>
    ...
    <Page>
      ...
      >
        <FormGrid>
          ...
          <Input
            label="Motivo"
            placeholder="Ex: Feriado, folga, reunião..."
            value={form.reason}
